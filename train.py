...
EPSILON_START = 1.0
EPSILON_END = 0.1
EPSILON_DECAY = 1000000  # Cambiado para explorar más
LEARNING_RATE = 1e-5
TARGET_UPDATE_FREQ = 100  # Ajuste para actualizaciones más frecuentes en pasos
    ...
    for episode in range(num_episodes):
        ...
        while True:
            ...
            if random.random() < epsilon:
                ...
                with torch.no_grad():
                    state_tensor = state.unsqueeze(0).reshape(1, 3, 84, 84)
            ...
            if steps_done % 4 == 0 and len(memory) >= BATCH_SIZE:
                ...
                state_batch = torch.stack(batch[0]).to(
                    device).resize(BATCH_SIZE, 3, 84, 84)
                action_batch = torch.tensor(
                    batch[1], dtype=torch.int64).unsqueeze(1).to(device)
                reward_batch = torch.tensor(
                    batch[2], dtype=torch.float32).unsqueeze(1).to(device)
                next_state_batch = torch.stack(batch[3]).to(
                    device).resize(BATCH_SIZE, 3, 84, 84)
                done_batch = torch.tensor(
                    batch[4], dtype=torch.float32).unsqueeze(1).to(device)

                # Implementación de Double DQN
                q_values = policy_net(state_batch).gather(1, action_batch)
                next_actions = policy_net(
                    next_state_batch).argmax(1, keepdim=True)
                next_q_values = target_net(next_state_batch).gather(
                ...
                nn.utils.clip_grad_norm_(
                    policy_net.parameters(), 1)  # Gradient clipping
                optimizer.step()
        ...
        if episode % 100 == 0:
            torch.save(policy_net.state_dict(),
                       f"pacman_dqn_episode_{episode}.pth")
...
train(env, policy_net, target_net, memory, optimizer, num_episodes=EPISODES)
